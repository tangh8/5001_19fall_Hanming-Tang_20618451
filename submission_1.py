...
def test_fct(data):
    ...
    result=predition.predict(data)
    resulting_test=pd.DataFrame(result,columns=['playtime_forever'])# the result
    resulting_test.to_csv('C:/Users/LENOVO/Desktop/resulting_test.csv',index_label=["id"])# please change the location of the file

if __name__ == '__main__':
    traindata=pd.read_csv("C:/Users/LENOVO/Desktop/train.csv") # please change the location of the file
    traindata.sort_values("total_positive_reviews",inplace=True,ascending=False)
    traindata["playtime_forever"][0:30]=traindata["playtime_forever"][0:30]*2.5 # increasing the weight of the factor "total_positive_reviews"
    testdata=pd.read_csv("C:/Users/LENOVO/Desktop/test.csv") # please change the location of the file
    ...
    features=x.keys()
    pca = PCA(n_components=35) #using pca to get the most important 45 factors
    pca.fit(x)
...
#computing the result on test data
    for fa in features:
